...
export class PaymentBreakdownComponent implements OnInit {
  ...
  @Input() recievedHotels
  @Input() recievedFlights;
  @Input() travelers
  ...
  constructor(
    ...
    private hotelBook: HotelBookingService,
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes.price || changes.meal || changes.baggage || changes.seat) {
      this.calculateSsrCost();
    }

    if (this.hotelCost) {
      this.totalCost =
        this.hotelCost + this.taxes + this.gst - this.incentiveEarned;
      ...
      this.totalCost =
        this.flightCost + this.taxes - this.incentiveEarned + this.ssrCost;
      this.transactionFee = this.totalCost * 0.0175;
      this.totalCost += this.transactionFee;
  ...
  calculateSsrCost() {
    ...
    this.costData.emit({
      ...
      txes: this.taxes,
      incentiveEarned: this.incentiveEarned,
      isPanReuired: this.isPanRequired,
      isPassportReuired: this.isPassportRequired,
    });
  }

  async getFlightDetails() {
    try {
      const res = await this.pack.getFlightDetails(
  ...
  async fareQuoteCall(resultIndex: string, token: string, traceId: string) {
    ...
    try {
      ...
      if (fareQuoteResponse) {
        ...
        await this.pack.saveFareQuote(
          this.fareQuote.FareBreakdown,
          sessionStorage.getItem("uid"),
          this.fareQuote.Source
        );
        this.flightCost = this.fareQuote?.Fare?.PublishedFare;
        this.incentiveEarned =
          this.fareQuote?.Fare?.CommissionEarned +
          this.fareQuote?.Fare?.IncentiveEarned +
          this.fareQuote?.Fare?.PLBEarned +
          this.fareQuote?.Fare?.AdditionalTxnFeePub;
        this.taxes =
          this.fareQuote?.Fare?.TdsOnCommission +
          this.fareQuote?.Fare?.TdsOnIncentive +
          this.fareQuote?.Fare?.TdsOnPLB;
        ...
        this.totalCost = +this.totalCost.toFixed(2);
        this.isPanRequired =
          this.fareQuote?.IsPanRequiredAtBook ||
          this.fareQuote?.IsPanRequiredAtTicket;
        this.isPassportRequired =
          this.fareQuote?.IsPassportRequiredAtTicket ||
          this.fareQuote?.IsPassportRequiredAtBook ||
          this.fareQuote?.IsPassportFullDetailRequiredAtBook;
        this.costData.emit({
          totalCost: this.totalCost,
          flightCost: this.flightCost,
          txes: this.taxes,
        ...
        await this.ssrCall(
          ...
          this.token,
          this.traceId,
          this.flightData.segments[0][0].Origin.Airport.CityCode,
          this.flightData.segments[0][this.flightData.segments[0].length - 1]
  ...
  async fareQuoteCall1(resultIndex: string, token: string, traceId: string) {
    ...
    try {
      ...
      if (fareQuoteResponse) {
        ...
        await this.pack.saveFareQuote1(
          this.fareQuote1?.FareBreakdown,
          sessionStorage.getItem("uid"),
          this.fareQuote1.Source
        );
        // Ensure proper initialization
        this.flightCost1 = this.fareQuote1?.Fare?.PublishedFare || 0;
        this.incentiveEarned1 =
          this.fareQuote1?.Fare?.CommissionEarned +
            this.fareQuote1?.Fare?.IncentiveEarned +
            this.fareQuote1?.Fare?.PLBEarned +
            this.fareQuote1?.Fare?.AdditionalTxnFeePub || 0;
        this.taxes1 =
          this.fareQuote1?.Fare?.TdsOnCommission +
            this.fareQuote1?.Fare?.TdsOnIncentive +
        ...
        this.netPayable1 =
          this.taxes1 + this.flightCost1 - this.incentiveEarned1;
        this.initialCost1 = this.netPayable1;

        this.isPanRequired =
          this.fareQuote1?.IsPanRequiredAtBook ||
          this.fareQuote1?.IsPanRequiredAtTicket;
        this.isPassportRequired =
          this.fareQuote1?.IsPassportRequiredAtTicket ||
          this.fareQuote1?.IsPassportRequiredAtBook ||
          this.fareQuote1?.IsPassportFullDetailRequiredAtBook;
        ...
        await this.ssrCall1(
          ...
          this.token,
          this.traceId,
          this.flightData.segments1[0][0].Origin.Airport.CityCode,
          this.flightData.segments1[0][this.flightData.segments1[0].length - 1]
            .Destination.Airport.CityCode
        );
      }
    } catch (error) {
  ...
  async fareQuoteCall2(resultIndex: string, token: string, traceId: string) {
    ...
    try {
      ...
      if (fareQuoteResponse) {
        ...
        await this.pack.saveFareQuote2(
          this.fareQuote2?.FareBreakdown,
          sessionStorage.getItem("uid"),
          this.fareQuote2.Source
        );

        // Ensure proper initialization
        this.flightCost2 = this.fareQuote2?.Fare?.PublishedFare || 0;
        this.incentiveEarned2 =
          this.fareQuote2?.Fare?.CommissionEarned +
            this.fareQuote2?.Fare?.IncentiveEarned +
            this.fareQuote2?.Fare?.PLBEarned +
            this.fareQuote2?.Fare?.AdditionalTxnFeePub || 0;
        this.taxes2 =
          this.fareQuote2?.Fare?.TdsOnCommission +
            this.fareQuote2?.Fare?.TdsOnIncentive +
        ...
        this.netPayable2 =
          this.taxes2 + this.flightCost2 - this.incentiveEarned2;
        this.initialCost2 = this.netPayable2;

        this.isPanRequired =
          this.fareQuote2?.IsPanRequiredAtBook ||
          this.fareQuote2?.IsPanRequiredAtTicket;
        this.isPassportRequired =
          this.fareQuote2?.IsPassportRequiredAtTicket ||
          this.fareQuote2?.IsPassportRequiredAtBook ||
          this.fareQuote2?.IsPassportFullDetailRequiredAtBook;
        ...
        await this.ssrCall2(
          ...
          this.token,
          this.traceId,
          this.flightData.segments2[0][0].Origin.Airport.CityCode,
          this.flightData.segments2[0][this.flightData.segments2[0].length - 1]
            .Destination.Airport.CityCode
        );
      }
    } catch (error) {
  ...
  async getRoomData() {
    try {
      const res = await this.hotelBook.getRoomData(
        sessionStorage.getItem("hotel_uid")
      );
      console.log(res);
      this.hotelData = res;
    } catch (error) {
  ...
  async hotelBlockRoom() {
    const payload = {
      ...
      HotelRoomsDetails: this.hotelData?.hotel_details?.rooms.map((item) => {
        return item.room;
      }),
