...
@Component({
  selector: 'app-flight-card-package',
  templateUrl: './flight-card-package.component.html',
  styleUrls: ['./flight-card-package.component.scss']
})
export class FlightCardPackageComponent implements OnInit {
  ...
  getFlightDuration(flightRankInArray: number): string {
    ...
    // if direct flight
    if (this.completeFlight.length === 1) {
     if(this.completeFlight[0].AccumulatedDuration){
