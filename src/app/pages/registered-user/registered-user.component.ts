...
export class RegisteredUserComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.mobile=this.userData[0].contactNumber

    this.info.contactNumber=this.userData[0].contactNumber
    this.info.merchantId=this.userData[0].merchantId
  ...
  async postdata(data:any){
    ...
    try {
      const res= await this.http.post("http://localhost:4000/crm/customerEnquiry",data).toPromise() as any
  ...
  enquiry(){
    this.router.navigate(["/customerEnquiry"],{queryParams:{user: this.mobile}})
  }
  display = "none";
  ...
  toggleEdit(i,inf:any) {
    ...
    this.router.navigate(["/customerEnquiry"],{queryParams:{user: this.mobile},  state: {info: inf, upd: true} })
  ...
  async flData(dat:any){
    try {
      const res= await this.http.post("http://localhost:4000/crm/flightsEnquiry", dat).toPromise()
