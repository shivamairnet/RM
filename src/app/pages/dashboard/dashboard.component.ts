...
saveResponseDataToFirestore() {
  ...
  //     airport.toLowerCase().includes(this.departureSearchText.toLowerCase())
  //   );
  // }
  ...
  try {
    ...
    if (!this.destinationSearchText || this.destinationSearchText.trim() === '') {
    ...
    // Determine which response arrived first
    const airportsFirst = responseAirports && (!responseCities || responseAirports.data.timestamp < responseCities.data.timestamp);

    // Update search results based on which response arrived first
    if (airportsFirst) {
    ...
    console.log(this.destinationSearchCities);
  } catch (err) {
  ...
  async onDepartureSearch() {
    try {
      ...
      console.log(this.departureSearchAirports);
      console.log(this.departureSearchCities);
    } catch (err) {
      console.error(err.message);
    }
  }
  ...
  findCityByName(cityName: string): City {
    return (
      this.cityList.find(
        (city: { cityName: string }) => city.cityName === cityName
      ) || new City("", "", "", [], "")
    );
  }
